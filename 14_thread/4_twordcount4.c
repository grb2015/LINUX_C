...
/************************************************************************************  
    breif :
            线程间通信。
            在计算单词的程序中，如果同时计算两个文件，一个文件很大，一个很小。我们需要那个小的
            一计算出来之后，就通知调用它的线程它的统计结果，如何实现呢？
			当然，我们可以在4_twordcount3.c中先pthread_join那么小的线程，退出之后立即打印。但是如果我们同时计算很多个文件呢?
			知道，最后一个必须等待前面所有的文件都计算完了它才能打印。就算你按文件的大小从小到大来jion，效率仍然是很低的。

            这里通过选举中心的例子,解释了互斥变量，锁,条件变量的使用.还是使用之前的单词统计的程序

   history: 2017-05-11 renbin.guo created
    
        usage: 这里可以多开一些线程，计算n个文件，以便看出效果
