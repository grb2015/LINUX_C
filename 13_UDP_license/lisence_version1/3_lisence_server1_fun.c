...
handle_request(char *req,struct sockaddr_in *client, socklen_t addlen)
{
	...
	// renbin.guo added 2017-05-06 for debug 
	printf("grb req = %s",req);
...
char *do_hello(char *msg_p)
{
	...
	num_tickets_out++;
	// renbin.guo added for debug
	printf("grb in do_hello return ");
	return(replybuf);
} /* do_hello */
...
narrate(char *msg1, char *msg2, struct sockaddr_in *clientp)
{
	...
	// 为什么这个打印不出来 ? printf()不能用吗?printf和fprintf不能同时用?并且客户端里面的narrate也要相应的修改成一致
	//printf("right !");	 //打印不出来

	// renbin.guo fixed bug  #9
	/*
	if ( clientp )
